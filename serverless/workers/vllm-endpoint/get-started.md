...
一旦 Endpoint 初始化完成,你就可以开始向你的 [Endpoint](/serverless/endpoints/get-started.md) 发送请求。请继续阅读 [发送请求](#sending-a-request) 部分获取进一步说明。
...
3. 提供以下详细信息:
   ...
   - (可选)挂载一个 [网络存储卷](/serverless/endpoints/manage-endpoints#add-a-network-volume)
   - 配置环境变量:
     - `MODEL_NAME`:(必需)指定大语言模型(例如 `openchat/openchat-3.5-0106`)
     - `HF_TOKEN`:(可选)为私有模型提供你的 Hugging Face API token
4. 点击 **Deploy**。

一旦 Endpoint 初始化完成,你就可以开始向你的 [Endpoint](/serverless/endpoints/get-started) 发送请求。请继续阅读 [发送请求](#sending-a-request) 部分获取进一步说明。

有关可用环境变量的完整列表,请参阅 [vLLM Worker 变量](/serverless/workers/vllm-endpoint/environment-variables) 文档。
...
| `batch_size_growth_factor` | `int` | 环境变量 `DEFAULT_BATCH_SIZE_GROWTH_FACTOR` | 每次调用时 min_batch_size 乘以的增长因子,直到达到 max_batch_size。 |
...
有关管理 Serverless Endpoint 的更多信息,请参阅 [管理 Endpoints](/serverless/endpoints/manage-endpoints) 指南。有关 vLLM Worker 环境变量的完整参考,请参阅 [vLLM Worker 变量](/serverless/workers/vllm-endpoint/environment-variables) 文档。
