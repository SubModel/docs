...
| `ROPE_THETA`                                     | None                 | `float`                                                      | RoPE theta。与 rope\_scaling 配合使用。                                |
...
| `SPEC_DECODING_ACCEPTANCE_METHOD`                | 'rejection\_sampler' | \['rejection\_sampler', 'typical\_acceptance\_sampler']      | 指定推测解码中草稿 token 验证的接受方法。                                        |
